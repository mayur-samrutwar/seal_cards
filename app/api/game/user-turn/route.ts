...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!selectedStat) {
      return NextResponse.json(
        { error: 'selectedStat is required' },
        { status: 400 }
      )
    }

    const validStats: StatName[] = ['size', 'armour', 'power', 'attack', 'speed', 'age']
    ...
    if (gameState.currentTurn !== 'user') {
      return NextResponse.json(
        { error: "It's not the user's turn" },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      { error: 'Failed to process user turn' },
      { status: 500 }
    )
  }
}
